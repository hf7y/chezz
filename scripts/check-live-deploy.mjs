...
// Classic DOES carry its own report box -- it has no "netlify" reference in
// its built artifact only because, until hf7y/chezz#128 (classic side), it
// pointed at the retired Google Apps Script URL (hf7y/chezz#82) instead,
// on BOTH hosts it's fully served from (unlike narrative's root, it isn't a
// redirect stub on either). Checked by page CONTENT, not just status --
// the dead Google URL returned 200 too.
...
function fileBlocker(failures) {
  ...
  // make), plus DEFERRED/DELIVERS blocks. Discovered broken tonight -- this
  // path had never actually filed an issue against the real grammar gate.
  const body = [
...
async function main() {
  ...
  if (failures.length === 0) {
    console.log(
      ...
      " all serving nightly-builds/, the report endpoint answers, Narrative's non-canonical" +
      " routes still redirect to Netlify, and classic's report channel names the live function."
    );
    try {
